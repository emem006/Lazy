...
function saveBind($uid,$data){
	...
	$obj=D('Bind');
	$check_res=M('Bind')->where(array('uid'=>$uid,'type'=>$data['type']))->find();
	if($check_res){
		return array('flag'=>'error','message'=>'已经绑定过，无需在操作'	);
	}else{
		$bind_data=array(
			'uid'				=>$uid,
			'openid'			=>$data['openid'],
			...
			'profileImageUrl'	=>$data['head'],
		);
		if(!empty($data['refresh_token']))$bind_data['refresh_token']=$data['refresh_token'];
